...
def proximos_dias(n=5):
    hoje = date.today()
    return [(hoje + timedelta(days=i)).strftime("%d.%m.%Y (%a)") for i in range(1, n + 1)]
    ...
    try:
        ...
        if msg.get("type") == "interactive" and msg["interactive"]["type"] == "list_reply":
            ...
            if "servico" not in sessao:
                ...
                enviar_lista(de, f"Perfeito, «{escolha}». Für welchen Tag?", "Termine", proximos_dias())

            elif "data" not in sessao:
                sessao["data"] = escolha
                enviar_lista(de, f"Am {escolha} — welche Uhrzeit passt Ihnen?", "Uhrzeiten", HORARIOS)

            elif "hora" not in sessao:
                sessao["hora"] = escolha
                resumo = (f"Name: {de}\nLeistung: {sessao['servico']}\n"
                          f"Datum: {sessao['data']}\nUhrzeit: {sessao['hora']}")

                enviar_texto(de, f"Danke! Ihre Anfrage:\n\n{resumo}\n\n"
                                  f"Wir bestätigen den Termin so schnell wie möglich.")

                if PROVIDER_WHATSAPP:
                    enviar_texto(PROVIDER_WHATSAPP,
                                 f"📅 Neue Terminanfrage über den Bot:\n\n{resumo}\n"
                                 f"Kunde: wa.me/{de}")
        ...
        # Qualquer mensagem de texto normal reinicia o fluxo
            ...
            sessoes[de] = {}
            enviar_lista(de, "Guten Tag! Willkommen bei Wrap Shop Test 🚗\nWelche Leistung möchten Sie buchen?", "Leistungen", SERVICOS)
